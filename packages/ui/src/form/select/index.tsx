...
const Select: SelectType = ({
  ...
  return (
    <div className={`min-w-48 relative w-full ${sizes[size]}`}>
      <select
        className={`${classes.select} ${error ? classes.selectError : classes.selectValid}
          `}
        placeholder=' '
        value={value}
        onChange={onChange}
      ...
      <label className={`${classes.label} ${labelSizes[size]} ${error ? classes.labelError : classes.labelValid} `}>
        {label}
      </label>
    </div>
  )
